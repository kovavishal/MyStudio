...
<body>
		...
		<div class="card">
			<div class="card-header text-center"
				...
				<jsp:include page="logout.jsp">
					<jsp:param name="page" value="Agent Report" />
				</jsp:include>
			</div>
 ...
 -->
						<button type="button" class="btn btn-success" onclick="report();"
							id="searchBtn" style="width: 200px; font-size: 20px">
							<i class="fa fa-search"
								style='font-size: 20px; padding: 0 1em 0 0'></i>Search
						</button>
							<a href="<%= request.getContextPath()%>/report/agent-remoteAccess"
					...
					<table id="reporttable"
						...
						<thead>
							<tr>
								...
								<th class="col-sm-1 col-lg-12" data-field="balance"
									nowrap="nowrap" align="right" hidden="true">Cust.No.</th>
								<th class="col-sm-1 col-lg-12"
									style="width: 20%; position: relative; text-align: center;"
									data-field="referenceType" nowrap="nowrap">Customer Name</th>
								<th class="col-sm-1 col-lg-12"
									style="width: 10%; position: relative; text-align: center;"
									data-field="trxnType" nowrap="nowrap">Sheets</th>
								<th class="col-sm-1 col-lg-12"
...
function report(){
	...
	document.title = 'Agent Reports '+' from :'  + $('#fromdate').val() +' To :'+ $('#todate').val(); 
	if(fromdate!='' && todate!=''){
	var reqData={
	...
	var tableClient = $('#reporttable').DataTable({
		...
		"searching":true,
		   "order": [[ 2, "asc" ]],
			...
			"ajax": {
			"url": "${pageContext.request.contextPath}/report/adetail_remoteAccess",
			"type": "POST",
			"data" : reqData,
		...
		"footerCallback": function ( row, data, start, end, display ) {
            ...
            $( api.column( 4 ).footer() ).html('<td bgcolor="#00BFFF";font-width="40px">'+
                    total+'</td>'
            );
